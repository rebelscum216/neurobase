...
- `raw_filename(captured_at: datetime, agent: str, session_id: str | None) -> str` — `{ts}_{agent}_{sid8}.md` where `ts` is `captured_at` in UTC formatted `%Y-%m-%dT%H-%M-%S.%fZ` (**microsecond** precision, ADR-0023). Ordering is *not* derived from the filename anymore — see `list_raw`.
- `raw_path(root, project, captured_at, agent, session_id) -> Path` — `memory_dir(project, root) / "raw" / raw_filename(...)`.
- `write_raw(root, project, *, agent, session_id, cwd, branch, captured_at, body, transcript_path=None, unique=False) -> Path` — writes (or session-keyed-overwrites) a raw capture. With `unique=False` (the default): computes the target via `raw_path`; if it exists **and** frontmatter `consumed` is truthy, raises `RawConsumedError` telling the caller to retry with `captured_at=now` (a fresh filename). With `unique=True` (the ADR-0023 contended-scribe path): claims a *fresh* filename via `_claim_fresh_raw_path` so the write cannot touch any raw a concurrent holder owns. The claimed name **always** carries a `__gNNNN` generation suffix (starting `__g0001`) and is **never** the bare `{ts}_{agent}_{sid8}.md` base — that base is reserved for the ordinary lock **holder** (P1-DATA-INTEGRITY-001): a contended writer that claimed the base while it was still absent would be overwritten by the holder's imminent atomic replace onto the same base. **`captured_at` is never modified**; it is written to frontmatter as-is, and a generation suffix (not a moved timestamp) provides uniqueness. This implements the spec §1 "mutability rule" (rewritable by the owning scribe until `consumed`) and its contended-capture rule (a scribe that cannot take the project lock writes a fresh raw rather than overwriting the one being consumed).
- `list_raw(root, project, unconsumed_only: bool = True) -> list[Document]` — globs `raw/*.md` (returns `[]` if the dir doesn't exist), sorted **oldest-*capture*-first by the authoritative `captured_at` frontmatter** (ADR-0023), with the filename as a stable tiebreak — *not* by filename string, which depended on `agent`/`sid8` bytes and inverted whenever one session's contended captures overlapped a later capture under another key. A raw with a missing/unparseable `captured_at` sorts first, by name. Skips any file that fails `read_doc` (`ValueError`/`OSError`) — unparseable or unreadable files are never fatal. If `unconsumed_only`, filters out documents with `consumed` truthy.
- `mark_consumed(path: Path, *, expect_digest: str | None = None) -> Path | None` — reads the doc, copies the frontmatter and sets `consumed = True`, rewrites via `write_doc` with the body untouched. Still "the only permitted mutation of an existing raw file" — every other field and the body are preserved verbatim. `expect_digest` is a **defensive guard, not a compare-and-swap** (ADR-0023): if given and the file changed since the caller read it, the function returns **`None` without marking it consumed**, so a capture the plan never saw stays retryable — callers must treat `None` as "not consumed." Correctness under concurrency comes from the per-project write lock the pass holds, not from this check.
...
- `soft_delete_curated(root, project, slug) -> Path` — validates slug, reads `curated/<slug>.md`, sets `status: "tombstoned"` and `tombstoned_at: <now>`, writes the doc (unchanged body) to `.tombstones/<slug>.md`, then unlinks the original, returning the tombstone path. This is a move via copy-then-delete rather than `Path.rename`, so it stays atomic per-file (each write is tmp+rename) even though the two-step move+unlink isn't itself a single atomic filesystem operation. Recoverable until pruned. **Failure semantics (spec §1):** a `FileNotFoundError` on the active unlink means a racing soft-delete already completed the move and is treated as *success*; any other unlink error propagates **and the tombstone is left in place**. The function never deletes `.tombstones/<slug>.md` — not even to undo its own failed move — because that path is shared and a racing delete may have replaced it with *its* in-flight tombstone; deleting that copy would lose the fact. The leftover is reader-invisible residue (see the placement rule below).
- **Placement rule — `curated/` is authoritative (spec §1).** Either or both of `curated/<slug>.md` and `.tombstones/<slug>.md` may exist; when both do, the slug is **active** and the tombstone is residue. `list_curated` reads only `curated/`, and the recommender's evidence resolver checks `curated/` before `.tombstones/`, so current readers satisfy this for free — but any *new* reader of `.tombstones/` MUST exclude slugs present in `curated/`. Nothing on a write path reclaims residue; only `prune_tombstones` does, once past the grace period. `prune_tombstones`'s TOCTOU against a concurrent soft-delete is **closed by the project write lock** (ADR-0023, known-gaps **G4**): it runs only inside the locked pass and is its sole call site, so no soft-delete can interleave, and the store's no-loss property now holds across concurrent mutating passes.
...
- `_SECRET_NAME` → `[A-Z0-9_]*(?:KEY|TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIAL)[A-Z0-9_]*` — the shared secret-ish variable-name fragment the env rules below build on.
- `_VALUE` → `(?:"(?:\\.|[^"])*"|'[^']*'|\$'(?:\\.|[^'])*'|\S+)` — a complete shell value: single-, double-, or ANSI-C-quoted, or a bare run. **Not `\S+`**, which was the original and is a leak: `api_token="hunter two"` matched only `"hunter` and left ` two"` sitting in the store in the clear. Quoted values with spaces are precisely what passwords and tokens look like. The assignment rules also accept a quoted *name* (`"api_token"=v` is valid shell for the same assignment).
...
- `_SHELL_SEGMENT` → multiline, case-insensitive `(?:^|(?<=[;&|(`+"`"+`]))([ \t]*(?:export|env|declare|typeset|local)\b[^\n;&|`+"`"+`]*)` → a shell keyword in **command position** (opening a line, or after a separator), matched through the end of that segment. Every secret-named assignment *inside the matched segment* is then redacted case-insensitively via `_scrub_assignments`. Two things are load-bearing here, and getting either wrong is a real bug we shipped and had to fix: **(a) position, not the keyword, is what proves shell syntax** — a rule keyed on the bare keyword redacts prose (`we export api_token=example in docs`) and SQL (`SQL DECLARE api_key=value`), gutting the technical content the richer skim exists to preserve; **(b) the whole segment, not the first assignment after the keyword** — real commands carry option operands and multiple assignments (`env -u OLD PATH=/bin api_token=… pytest`), and scrubbing only the first token silently leaves the rest exposed. `setenv` is deliberately absent from the keyword list: its syntax is `setenv NAME value`, with no `=`, so it could never match an assignment rule anyway.
- `_INLINE_ENV_SECRET` → `(?<![A-Za-z0-9_])(<SECRET_NAME>)[ \t]*=[ \t]*\S+` → a **bare** inline assignment with nothing to disambiguate it: `API_TOKEN=… cmd`, `make && API_TOKEN=…`. Here the *name's shape* is the only signal that this is a secret rather than a keyword argument, so this rule is deliberately **case-sensitive**: a case-insensitive version would swallow `items.sort(key=lambda x: x.id)` and `df.groupby(key=col, secret=False)`.

**`_scrub_shell(text)`** — the shell scrubber, used by both the `_SHELL_SEGMENT` rule and `redact_command`. **It deliberately does not model command position.** The obvious rule — "an assignment only counts before the command name" — needs the whole POSIX command grammar (pipelines, wrappers and their long-option operands, redirections, `env`'s grammar, substitutions, `\`-newline continuations), and six successive implementations of it each shipped a *secret leak*, because an approximation of that grammar fails **open**. What replaced it:

> In **unquoted** shell text, redact the value of every secret-named assignment, wherever it appears. Never touch a **quoted** argument — except to recurse into command substitutions, which the shell executes. **Heredoc bodies** are data.

That needs only the three things a lexer gets right, and `_scrub_shell` walks the text once tracking exactly those:
- **Single quotes** are inert; **double quotes** are inert *except* for `$(…)` and backticks, which the shell still executes inside them — `_scrub_double_quoted` recurses into those and nothing else, so `python -c "items.sort(key=lambda x: x.id)"` survives verbatim while `echo "$(api_token=… ./run)"` cannot hide a secret one quote deep.
- **Heredoc bodies** (`_copy_heredoc_body`) are copied verbatim: they are a file, a script, SQL. All heredocs a line declares are queued and consumed *in order*, and the terminator must match **exactly** (`<<-` strips leading TABS only, per POSIX). `_HEREDOC_OP` is matched only in *unquoted* text — a `<<` inside a quoted argument is a bit-shift, and promoting it to a heredoc hid the entire next command from redaction.
- **Backslash escapes**, including `\<newline>` continuations, keep a value glued to its assignment across physical lines.
- Everything else is unquoted shell: a secret-named assignment at a word boundary (`_ASSIGN_HEAD`, name may itself be quoted) or a credential option (`_OPTION_HEAD` + an allow-list) has its whole value consumed by `_skip_value` and replaced.

**Idempotence is a contract, not a nicety.** `redact` runs more than once over the same text — the scribe scrubs each captured value, then the whole assembled document as defense in depth. `[REDACTED:env-secret]` contains no word-break character, so a value scanner that doesn't recognize the marker reads `[REDACTED:env-secret]export …` as a single bare token and replaces it, **silently eating the following word**. So a marker is matched as a value — but only when it is the *complete* value: a marker *prefix* (`api_token=[REDACTED:env-secret]SECRET`) must still redact the suffix, or a marker pasted into captured text becomes an escape hatch for the secret beside it. Configured `extra_patterns` obey the same rule: existing markers are opaque, nonempty custom matches and built-in redactions run to one combined fixed point, and zero-width matches are ignored because they contain no captured text. Both marker/value bugs were real, and the first was caught by the idempotence check in `scripts/audit_command_redaction.py` — the one real property that script can test without an oracle.

**A value is a RUN of fragments** (`_VALUE_FRAGMENT`+), because the shell concatenates adjacent quoted and bare fragments into one word: `api_token="a b"tail` is a single value. Matching one balanced fragment and stopping leaked the rest of the secret *and* left the marker abutting text for the next pass to swallow — non-idempotence and a leak from one root cause.

**There is no executor inference, by design.** `eval '…'` and `sh -c "…"` do execute their string argument, and two attempts to detect that both leaked and mangled: keying on any `sh`/`bash` token destroyed `echo sh -c '…'` (which executes nothing), and keying on command position still missed `env sh -c '…'` / `command sh -c '…'` while over-arming `sh -- -c '…'` (after `--`, `-c` is the command *name*). Recognizing an executor is the POSIX command grammar wearing a smaller hat. So **a quoted argument is always data**, with no exception, and the residual — a secret inside an eval'd string — is declared in spec §10 and pinned by `test_executed_string_residual_is_known_and_bounded`.

**`_scrub_substitution`** exists for one reason worth stating: **redaction must never delete captured input.** The scanners are heuristics and will disagree with a real shell — a lone apostrophe inside a heredoc body makes `_end_of_substitution` run to end-of-text — and an unconditional `end - 1` slice silently ate the final character of a real `git commit -m "$(cat <<'EOF' … )"`. It re-emits a closing delimiter only when one was actually there. A mis-scan is acceptable; losing a byte of the user's command is not.

**The accepted cost.** A credential-*named argument* to another command (`env PATH=/bin pytest api_key=example`, `awk -v api_key=x`) is redacted even though a perfect parser would leave it alone. It is fail-closed, the command's shape survives (`api_key=[REDACTED:env-secret]`), and only a small minority of real captured commands contain a secret-named `name=` token at all — so position tracking buys little fidelity in practice while costing correctness we could not deliver. The case for the trade is the **fail-open history** (seven revisions, each leaking a secret), not a percentage; `scripts/audit_command_redaction.py` re-runs the local no-mangling check and documents its method, but it is anecdotal validation against one corpus, not a general guarantee. `_SECRET_OPTION_NAMES` is an allow-list, never a `*key*` pattern: `--sort-key=name`, `--key=id`, `--password-policy=strict` select and configure, they do not authenticate.

**`redact_command(text, extra_patterns)`** — the public entry point for a value the caller *knows* is a shell command: spec §4's tool-activity digest, where the scribe captured Claude's `input.command` verbatim. It applies the same literal built-in patterns as `redact`, then uses `_scrub_shell` for assignment-shaped shell text, and composes that with configured extras to a fixed point. `adapters/claude/scribe.py` uses it for the `### Commands run` bullets and plain `redact` for everything else.

**Known residual** (recorded in spec §10): a lowercase secret-named assignment embedded *mid-sentence in prose* — "…then I ran export api_token=abc123" — is not redacted, because catching it requires treating any keyword anywhere as shell, which is the over-broad rule above. D13 is a best-effort regex table; silently gutting captured prose was judged the worse failure mode. Commands themselves are fully covered.
...
```
Applies every builtin pattern in table order, then the env-secret rule, then any caller-supplied `extra_patterns` (raw regex strings, typically sourced from config `[redact].extra_patterns`), each of which redacts its match to the fixed string `[REDACTED:custom]`. The built-in table and extras are iterated to one combined fixed point because a custom replacement can expose a built-in assignment boundary. Returns the fully redacted text; pure function, no I/O, no exceptions expected from well-formed input.
...
- `DEFAULT_PLAN_MAX_RAWS = 3` — spec §8's `PLAN_MAX_RAWS`; the per-batch **raw-count** budget, the second ceiling beside the byte budget, overridable via `[curate].plan_max_raws`. Bytes are not a proxy for it: a batch can sit far under the byte cap and still extract nothing because too many raws compete for one plan call (`known-gaps.md` G19).
- `DEFAULT_PLAN_TRIGGER_RAWS = 3` — spec §8's `PLAN_TRIGGER_RAWS`; the backlog size that arms `--if-stale` independently of `stale_hours`, overridable via `[curate].plan_trigger_raws`.
...
- `CURATOR_LOG = store.CURATOR_LOG` — per-project append-only pass log under the memory dir. The canonical definition lives in `core.store` (so a core-layer reader can share the name without a cross-layer format dependency, ADR-0022); this is a back-compat alias, so `engine.CURATOR_LOG` still resolves.
- `PLAN_SYSTEM` / `NODE_SYSTEM` — the two hand-written system prompts satisfying spec §2.1 and §2.2 respectively. `PLAN_SYSTEM` establishes the curator's identity and goal ("small, non-redundant, current fact set — optimize for deletion and merging, not accumulation"), states the reuse-slug/supersedes/tombstone/pinned rules, instructs that each upsert's `from_raw` cite "only filenames present in this request's raw_captures" (the prompt half of B1 — the deterministic filter in `_apply_upserts` is the actual enforcement, per spec §2.1), and mandates a bare-JSON response of the exact `{"upserts": [...], "tombstones": [...]}` shape. The response schema is unchanged by Slice B. `NODE_SYSTEM` mandates a single synthesized markdown node — title line plus grouped bullets, "invent nothing," markdown only.
...
- `curate(root: Path, project: str, brain: Brain, *, dry_run: bool = False, resynth: bool = False, tombstone_grace_days: int = DEFAULT_TOMBSTONE_GRACE_DAYS, plan_payload_max_bytes: int = DEFAULT_PLAN_PAYLOAD_MAX_BYTES, plan_max_raws: int = DEFAULT_PLAN_MAX_RAWS, distill: str = DEFAULT_DISTILL, distill_chunk_chars: int = DEFAULT_DISTILL_CHUNK_CHARS, redact_patterns: tuple[str, ...] = ()) -> dict[str, Any]` — runs one full pass of the spec §2 sequence and returns the summary dict (the function re-exported as `curate` from `neurobase.curator`, imported by the CLI as `run_curate`). It also accepts `blocking_lock: bool = True`. **The whole pass runs under the per-project write lock** (ADR-0023): `curate` is a thin wrapper that opens the WRITE handle, acquires `core.lock.project_lock(handle.memory_dir(project), blocking=blocking_lock)`, and delegates to the real body (`_curate_unlocked`, which takes the handle). `blocking_lock=False` — the detached `curate --if-stale` freshness pass — returns `{"status": "skipped-locked"}` instead of queuing if another writer holds the lock. Control flow (inside the lock):
  ...
  5. **Batch loop (ADR-0012 / decision D22)**, repeated until every raw is planned. Each iteration reloads active curated facts (`store.list_curated`) so a later batch sees everything earlier batches upserted, superseded, and tombstoned, then calls `_next_plan_batch` to take the next oldest-first run of raws under **both** ceilings — whose *final combined request* (`combine_prompt(PLAN_SYSTEM, user_payload)`, measured in UTF-8 **bytes**, exactly what a CLI backend passes as one argv entry) fits `plan_payload_max_bytes` (default 262,144), **and** which holds at most `plan_max_raws` raws (default 3). Whichever binds first ends the batch. The count ceiling exists because the byte cap does not constrain the thing that actually limits extraction: ten raws totalling 63,511 bytes pass the byte cap untouched and, as one batch, produced new facts in 0 of 6 measured runs (`known-gaps.md` G19). A non-positive `plan_max_raws` raises `ValueError`, like a non-positive byte budget. A raw too large to fit even alone is truncated by `_truncate_raw_to_fit` and marked, never skipped. If not even a marked envelope fits (an absurdly small budget, or curated facts that already fill it), `_next_plan_batch` raises `ValueError` and the pass returns `{"status": "error", …}` with nothing consumed.
  ...
  9. **Apply upserts** (step 4 of spec §2) via `_apply_upserts`, passing the set of raw filenames in *this* batch so `from_raw` can be validated against it (B1, ADR-0022). Then, iterating the returned `superseded_by` map (old slug → the landed slug that supersedes it, first-occurrence order), tombstone every superseded slug that was *not itself re-upserted in this batch* and is not pinned, via `_safe_soft_delete`, recording each applied removal in the pass's fold accumulator.
  10. **Apply explicit tombstones** (step 5): for each `{"slug", "reason"}` entry, skip if the slug is empty, was upserted in this batch, or is pinned; otherwise soft-delete it, count it, and record it in the fold accumulator.
  11. **Consume this batch's raw** (step 6): `handle.mark_consumed(doc.file_path, expect_digest=doc.digest)` for every doc in the batch — reaching this point means *that batch's* plan parsed successfully (even if empty). The whole pass runs under the per-project write lock (ADR-0023), so this read-then-write is safe; `expect_digest` is a defensive guard (not a compare-and-swap) that returns `None` if the raw changed since it was planned — a `None` result is skipped and kept out of the fold accumulator, so a capture the plan never saw is never marked consumed. Each *actually*-consumed raw's identity (`file`, `session_id`, `agent`, `captured_at`) is captured **at consumption time** into the fold accumulator, so a journal edge outlives any future raw-retention change. Loop back to 4 with the remaining raws.
  12. **Reconcile the fold's removal lists** against pass-end reality (ADR-0022). The per-batch guards in 9/10 only see the *current* batch's upserts, so with D22 batching a slug removed by an earlier batch and re-upserted by a later one is active at pass end — the pass did not, on balance, remove it. The active set (`store.list_curated`) is the oracle: any `fold.superseded`/`fold.tombstoned` entry whose slug is active at pass end is dropped and its count decremented, keeping each count equal to the length of its fold list. This repairs the **journal only** — the resurrected slug's old tombstone stays on disk, because spec §1 forbids unowned deletion of `.tombstones/<slug>`.
  13. **Prune tombstones** (step 7), once, after the loop: `handle.prune_tombstones(project, older_than_days=tombstone_grace_days)`. This is the sole `prune_tombstones` call site, and because the whole pass holds the project write lock (ADR-0023), its former TOCTOU with a concurrent `soft_delete_curated` is closed structurally — no other pass can mutate this project's `.tombstones/` mid-prune.
  14. **Synthesize** (step 8), once, after the loop: call `_synthesize(root, project, brain)` inside a `try/except Exception`. On *any* exception — brain error, malformed sibling node breaking the index rebuild, linkify/disk error — `synth_error = str(exc)`; this is caught deliberately broadly (`# noqa: BLE001`) per spec §2's partial-failure contract: the applied curated-fact state and raw-consumption already happened and must stand, because the node is a pure function of `curated/` and will self-heal on the next `curate` (or `--resynth`) pass.
  15. Build and return the summary dict: `{"status", "raw", "batches", "upserts", "superseded", "tombstones", "dropped_from_raw", "pruned_tombstones", "active_facts"}` (plus the distill counters `distilled`/`fallback` merged in, and `"error"`/`"synth_error"` on failure paths) — this key set matches spec §2 step 9. `upserts`/`superseded`/`tombstones`/`dropped_from_raw` are summed across batches. `status` is `"error"` if a batch's plan failed (the pass failed; its raws are still unconsumed), else `"partial"` if only synthesis failed, else `"ok"`. If a plan failure *and* a synthesis failure coincide, `error` carries the plan failure and `synth_error` the other, so neither masks the other. The summary is appended to the curator log via `_log_pass` before returning.
  16. **Fold record** (ADR-0022 B2). Whenever at least one batch committed — so it rides the after-commit *error* path too — `_log_pass` is given a `fold` object: `{v, consumed[], edges{}, superseded[], tombstoned[]}`, built from the accumulators in 9–12. It is deliberately **not** a summary key: the CLI prints the returned summary as JSON and `fold.consumed` carries raw `session_id`s, so the fold rides the on-disk journal record *only* (loopback-only sensitivity). It is absent when nothing committed (first-batch abort, noop) and on every dry-run path. Note this gate is **stricter than steps 13/14's**, not identical to it: prune and synthesis are guarded only by the `plan_error is not None and batch_count == 0` early return, so a *bounded* zero-commit stop (budget exhaustion on the first plan call, which leaves `plan_error` unset) still prunes and synthesizes — idempotent housekeeping over `curated/` — while writing no fold, because an empty fold would assert a pass that consumed nothing rather than saying nothing at all. ADR-0022 records the reasoning.

- `is_stale(root: Path, project: str, hours: int, trigger_raws: int = DEFAULT_PLAN_TRIGGER_RAWS) -> bool` — decision D8's `--if-stale` gate, with **two independent arms; whichever fires first wins**. The **count arm** returns `True` as soon as the unconsumed backlog reaches `trigger_raws` (default 3), regardless of age — without it the time gate guarantees a backlog has piled up before anything folds it, which is exactly the condition G19 measures as extracting nothing. The **time arm** then scans unconsumed raw docs; for each with a `captured_at` frontmatter field, parses it as ISO-8601 (tolerating a trailing `Z` by rewriting to `+00:00`) and returns `True` as soon as one predates `now - hours*3600` seconds. Docs missing `captured_at` are skipped (not counted as stale); docs with an unparseable timestamp are also skipped (`except ValueError: continue`) rather than raising. Returns `False` if the backlog is below `trigger_raws` **and** nothing unconsumed is old enough (including the case of zero unconsumed raw). The count arm reads unconsumed raws only, so a pass never re-arms the next one by consuming raws. A pure count arm would starve the tail — two sessions then silence never reach the threshold — which is why the time arm is not redundant. The CLI (`src/neurobase/cli/__init__.py:202`) calls this only when `--if-stale` is passed and `--resynth` is not (`checking_staleness = if_stale and not resynth`), using `config.curate.stale_hours` (spec §8 default: 12h) as `hours` and `config.curate.plan_trigger_raws` (default 3) as `trigger_raws`; if not stale it prints "Not stale — nothing to curate." and returns before `resolve_brain`, so it never constructs a `Brain` or calls `curate()`.
...
- `_ApplyResult` (frozen dataclass) — what one batch's upserts produced: `upserted: set[str]` (slugs that landed), `superseded_by: dict[str, str]` (old slug → the landed slug superseding it, first-occurrence order), `edges: dict[str, list[str]]` (landed slug → its validated raw filenames, for the fold journal), and `dropped: int` (hallucinated `from_raw` names discarded).

- `_apply_upserts(root: Path, project: str, upserts: list[dict[str, Any]], batch: set[str]) -> _ApplyResult` — implements spec §2 step 4. For each upsert entry: strips slug/body, skips the entry entirely if either is empty; filters `supersedes` of self-references and empties; filters `from_raw` of empties and then **validates it against `batch`** — the set of raw filenames actually shown to the model in this batch — so `provenance = [f"raw/{name}" for name in from_raw if name in batch]` (B1, ADR-0022). With D22 batching the model only ever sees the current batch's raws, so a cited filename outside that set is a hallucination — commonly an echo of an old raw basename appearing in a fact body's `## Lineage` block — and is dropped *before* it reaches the permanent `provenance` record, which is the exact channel the recommender's ranker treats as scoring ground truth. It then calls `store.upsert_curated(...)`, catching `store.InvalidSlugError` and skipping (the model occasionally emits a bad slug). For each upsert that lands it records the slug, adds `len(from_raw) - len(validated)` to `dropped`, unions the validated names into `edges[slug]` order-preservingly, and `setdefault`s each superseded slug into `superseded_by`. The caller (`curate`) is responsible for actually tombstoning the superseded slugs, applying the "unless re-upserted or pinned" exception, and for the pass-end reconciliation.
...
- `_log_pass(root: Path, project: str, summary: dict[str, Any], *, fold: dict[str, Any] | None = None) -> None` — appends `{**summary, "at": <UTC ISO-8601, "Z"-suffixed>}` as one JSON line to `<memory>/.curator-log.jsonl`, creating the memory directory if needed, and adds a `"fold"` key to that **record** when `fold` is given. The fold is a keyword argument rather than a summary key on purpose (ADR-0022): the CLI prints the returned summary as JSON and the fold carries raw `session_id`s, so it must never reach a printed surface — the journal line is its only home. Called on the noop, error, resynth, and final (`ok`/`partial`) `curate()` return paths; `--dry-run` returns before any `_log_pass` call on the success path, so dry runs leave no trend-log entry (a dry run that hits a plan error does log an `error` summary, without a fold — nothing committed).
...
- `scribe(root: Path, *, transcript_path: Path, cwd: str, reason: str, session_id: str = "") -> Path | None` — the entry point invoked by the CLI's hook dispatcher. Control flow, in order:
  ...
  9. Delegates to `lock.write_raw_guarded(open_store(root, StoreMode.WRITE), project, agent="claude", session_id=sid, cwd=str(resolve_cwd), branch=parsed["branch"], captured_at=datetime.now(UTC), body=body)` (ADR-0023) and returns whatever `Path` it returns. `write_raw_guarded` takes the per-project lock **non-blocking**: uncontended it does the ordinary session-keyed write; if a curate pass holds the lock it writes a fresh raw (`unique=True`, `captured_at=now`) rather than risk overwriting the raw being consumed. The hook never blocks and never fails on the lock.
...
- `block(text: str) -> str` — escapes **both** CommonMark heading syntaxes in a captured value. `_ATX_HEADING` catches the leading `#` run of a line (`## foo` → `\## foo`); `_SETEXT_UNDERLINE` catches a line of only `=` or `-`, which underlines the line above it and promotes *that* line to a heading retroactively (`\===`, `\---`) — the easier of the two to miss, because nothing about the promoted line looks like a heading, and escaping only ATX leaves the hole open. Escaping the underline also defuses the same line read as a thematic break. Captured content is untrusted markdown: a prompt, an assistant message, or an IDE context block can contain its own headings, and rendered as-is they become *the raw document's* sections — session content forging the structure the curator then reads. Indentation closes neither syntax: CommonMark still parses a heading indented up to three spaces, so escaping is the load-bearing part. Used for section bodies (the final summary, Codex's IDE context), for the hook-supplied `reason`, and — via `bullet` — for every list item.
...
- `HEADER` — the format-string framing header injected before any node bodies:
  ...
  **The "proven" claim is scoped, and the scope matters.** What spec §3 calls "proven, reuse the spirit" — and what ADR-0005 actually live-verified — is the *defensive framing and the delivery mechanism*: the first four sentences, which frame injected memory as background context rather than instructions (it matters because a naively-worded injection could be mistaken by the model for a directive), delivered as `hookSpecificOutput.additionalContext`. The closing retrieval-fallback sentence is **not** covered by that verification; it is an unevaluated retrieval instruction added later, and no live experiment has measured whether models act on it (review F3).

  That closing sentence comes from `docs/notes/2026-07-27-context-loading-improvements.md`, Phase 5 / annotation C-11. The failure it targets is a reading agent treating the synthesized node as exhaustive and answering "we don't have that in memory" when the fact is sitting in curated storage, merely un-synthesized. Steps 2–4 of that phase are agent behavior, not Neurobase code — the `memory_search` MCP tool already exists, so the entire mechanism here is instruction text, and the testable contract is only that the text is emitted (spec §3 is deliberately normative about the emitted header, not about model compliance). "(when available)" is load-bearing: injection happens through the `SessionStart` hook, which is independent of whether the agent has the Neurobase MCP server wired, so the header must not assume the tool is callable.
...
- `_assemble(header: str, bodies: list[str], cap: int = MAX_CONTEXT_CHARS) -> str | None` — greedily concatenates `header + _JOINER + body` for each body in order, checking after each append whether the running string still fits under `cap`. The moment a candidate body would overflow the cap: if it's the **first** body being tried (`i == 0`) and it alone (with header) still overflows, only the **body** is truncated — to `room = cap - len(header) - len(_JOINER)` chars, never the header; otherwise (a later body) that body and all subsequent ones are dropped entirely, and the loop breaks. This implements spec §3's rule precisely: "drop whole trailing nodes rather than truncating mid-node (truncate only if a single node alone exceeds the cap)."

  **The header is indivisible, so the function can decline to assemble at all.** When `bodies` is empty, or when `room < 1` — the cap cannot fit the whole header plus at least one character of node body — it returns **`None`**, and `emit()`'s existing falsy check injects nothing. The reasoning (spec §3, review F2): the header carries the trust framing that tells the reading agent this is background data rather than instructions, so a fragment sliced mid-sentence through that framing is worse than no injection — and a header-only payload is useless regardless, being framing for content that isn't there. The earlier implementation sliced the whole assembled string at `cap`, which for a long `memory_dir` path emitted a truncated header and zero node characters.
...
- `scribe(root: Path, *, rollout_path: Path, cwd: str = "", session_id: str = "") -> Path | None` — the entry point, called once per `Stop`/`notify` hook firing. Control flow:
  ...
  9. Calls `lock.write_raw_guarded(writer, project, agent="codex", session_id=sid, cwd=str(resolve_cwd), branch=parsed["branch"], captured_at=started, body=body)` (ADR-0023 — the same non-blocking lock guard as the Claude scribe, taking the WRITE handle opened just above). If this raises `store.RawConsumedError` — the curator already folded this session's raw mid-session and flipped `consumed: true` — the scribe **retries once** with `captured_at=datetime.now(UTC)`, producing a **new** raw file under a fresh filename (per the spec §1 raw mutability rule). The guarded path and the `RawConsumedError` retry are complementary: the former handles a pass *holding the lock*, the latter a pass that *already finished and consumed* the raw.
...
- **`curate(root, cwd, if_stale: bool = False, dry_run: bool = False, resynth: bool = False) -> None`** — folds unconsumed raw captures into curated facts (spec §2). Resolves config/root/cwd/project (same not-enabled-⇒-exit-1 pattern as `status`), checks schema. If `--if-stale` is set *and* `--resynth` is not, checks `is_stale(root, slug, config.curate.stale_hours, config.curate.plan_trigger_raws)`; if not stale, echoes "Not stale — nothing to curate." and returns without calling the brain at all (this is what makes the SessionStart hook's detached `spawn_curate_if_stale` cheap on the common case). Otherwise resolves a brain backend via `resolve_brain(config)`; `None` ⇒ red error naming `resolution.reason` and pointing at `neurobase doctor`, exit 1. Calls `run_curate(root, slug, brain, blocking_lock=not if_stale, dry_run=dry_run, resynth=resynth, tombstone_grace_days=config.curate.tombstone_grace_days, plan_payload_max_bytes=config.curate.plan_payload_max_bytes, plan_max_raws=config.curate.plan_max_raws, distill=config.curate.distill, distill_chunk_chars=config.curate.distill_chunk_chars, redact_patterns=tuple(config.redact.extra_patterns))`. `--dry-run` prints the preview as indented JSON — `summary["plans"]` (the list) when the backlog needed more than one batch, else `summary["plan"]` — and returns (no mutation happened inside the curator either; distill cache writes are disabled). Otherwise prints the summary minus the `"plan"` key as compact JSON, and exits 1 if `summary["status"] == "error"`.
...
**`claude_memory_dir(project_root: Path) -> Path`** — returns Claude Code's per-project auto-memory dir: `~/.claude/projects/<encoded-cwd>/memory/`, where the encoding is the sibling **`encode_project_path`**. That function rewrites **every character outside `[A-Za-z0-9-]`** to `-`, character-for-character, preserving an existing `-`; the rule was established by direct experiment and cross-checked against 33 ground-truth pairs (spec §12.3). Uses `project_root.as_posix()` rather than `str()` so the substitution also normalizes Windows backslash paths. ⚠️ Two earlier descriptions here were wrong the same way (G9): first `'/'` only, called "live-verified" against a sample path with no dot and no space; then `'/'`, `'.'` and space, derived from 33 samples that happened to contain no underscore. Both would have silently missed real paths — the second would still miss every `my_project`. A missing derived directory is now reported in `SeedResult.skipped`, naming the path searched, so a wrong derivation is visible rather than success-shaped.
