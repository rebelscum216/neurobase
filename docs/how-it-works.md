...
**`claude_memory_dir(project_root: Path) -> Path`** — returns Claude Code's per-project auto-memory dir: `~/.claude/projects/<encoded-cwd>/memory/`, where the encoding is the sibling **`encode_project_path`**. That function rewrites **`/`, `.` and ASCII space** to `-`, character-for-character, preserving an existing `-`; the class was derived from 33 ground-truth pairs (spec §12.3) and is *observed*, not specified. Uses `project_root.as_posix()` rather than `str()` so the substitution also normalizes Windows backslash paths. ⚠️ The earlier description here said only `'/'` → `'-'` and called it "live-verified" — wrong, and the cause of G9: any path holding a dot or a space (on a machine with a dotted username, every path) resolved to a directory that does not exist, and the import reported success while importing nothing. A missing derived directory is now reported in `SeedResult.skipped`, naming the path searched.
