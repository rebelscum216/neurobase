...
- `curate(root: Path, project: str, brain: Brain, *, dry_run: bool = False, resynth: bool = False, tombstone_grace_days: int = DEFAULT_TOMBSTONE_GRACE_DAYS, plan_payload_max_bytes: int = DEFAULT_PLAN_PAYLOAD_MAX_BYTES, distill: str = DEFAULT_DISTILL, distill_chunk_chars: int = DEFAULT_DISTILL_CHUNK_CHARS, redact_patterns: tuple[str, ...] = ()) -> dict[str, Any]` — runs one full pass of the spec §2 sequence and returns the summary dict (the function re-exported as `curate` from `neurobase.curator`, imported by the CLI as `run_curate`). Control flow:
  ...
  11. **Consume this batch's raw** (step 6): `store.mark_consumed(doc.file_path)` for every doc in the batch — reaching this point means *that batch's* plan parsed successfully (even if empty). Each consumed raw's identity (`file`, `session_id`, `agent`, `captured_at`) is captured **at consumption time** into the fold accumulator, so a journal edge outlives any future raw-retention change. Loop back to 4 with the remaining raws.
  ...
  13. **Prune tombstones** (step 7), once, after the loop: `store.prune_tombstones(root, project, older_than_days=tombstone_grace_days)`.
