...
**`_lex_shell(text)`** — a real (small) shell lexer, replacing the regex tokenizer that preceded it. It yields lossless `("word"|"sep"|"gap", text)` pieces: quoted spans are held *inside* their word (so `a="b c"` is one word and a `;` inside quotes is not a separator), and `;` `&&` `||` `|` `&` newline `(` `)` are emitted as real separators rather than being swallowed as word characters — which is what the position model below needs to see. An **unterminated** quote consumes to end of text, which makes malformed input **fail closed**: `api_token="hunter two` is redacted whole instead of leaking ` two`. A captured command that failed to parse is still a captured command, and can still carry a live credential.

**`_scrub_shell(text)`** — steps *around* heredoc bodies (`<<EOF` … terminator). A heredoc body is a file, script, or SQL blob the command consumes — its lines are not shell words, so `key=lambda …` in a Python heredoc is not an assignment. The rest of the D13 table still runs over it (`cat > .env <<EOF` is exactly where secrets live); only the assignment walker skips it.

**`_scrub_shell_words(text)`** — the position model, and the part that has been wrong the most times. A word is an assignment only in *assignment position*, and that position is tracked **per command**:
- Every separator starts a fresh command, reopening assignment position — a secret can sit in the prefix of *any* command in a pipeline or list (`echo ok; api_token=… ./run`), not just the first.
- **Wrappers** (`sudo`, `command`, `nohup`, `nice`, `exec`, `timeout`, `doas`, `stdbuf`) run another command, so the real command hasn't started and assignment position survives them.
- **Assignment builtins** (`export`, `declare`, `readonly`, `typeset`, `local`) take a word list that is assignments all the way down.
- **`env` has its own grammar** — options, option operands (`-u OLD`), assignments, then a COMMAND, after which the words are that command's arguments. This asymmetry is why a single `builtin_seen` boolean is wrong in *both* directions: it leaks `env -u OLD api_token=…` if it closes too early, and it corrupts `env PATH=/bin pytest api_key=example` (that `api_key` is pytest's argument) if it never closes.

`_is_secret_option` matches an **allow-list** of credential option names (`--api-key`, `--token`, `--client-secret`, `--password`, …) in any position, since the name announces the value. It is deliberately not a `*key*` pattern: `--sort-key=name`, `--key=id`, and `--password-policy=strict` are selection and policy flags, and mangling them destroys the digest for no security gain. `_redact_assignment` keeps the name verbatim, quoting and all, and drops the whole value token.

**`redact_command(text, extra_patterns)`** — a second public entry point, for a value the caller *knows* is a shell command: spec §4's tool-activity digest, where the scribe captured Claude's `input.command` verbatim. It runs `redact`, then `_scrub_shell` — no keyword needed, since the channel already proves the text is shell.

The thing to understand here, because it was a shipped bug: **a shell command is not "not prose and not code."** `python -c "items.sort(key=lambda x: x.id)"`, `sqlite3 db "DECLARE api_key=value"`, and `echo "we export api_token=… in docs"` all carry code, SQL, and prose as **quoted arguments**. A substring scan for secret-named assignments mangles all three — the `groupby` command loses its keyword arguments and its closing paren — while redacting no secret at all, which destroys exactly the activity signal the Tier-1 digest exists to capture. That is why assignments are found by *token and position* rather than by regex over the whole string. `adapters/claude/scribe.py` uses `redact_command` for the `### Commands run` bullets and plain `redact` for everything else.
