...
def _rule_start_marker(slug: str) -> str:
    """The full opening comment written at the head of a slug's managed rule
    block. Only its **sentinel** prefix (see `rule_sentinel`) is the identity
    token; the parenthetical after it is explanatory prose for whoever opens the
    file, and is deliberately NOT part of what identifies the block."""
    return (
...
def rule_sentinel(slug: str) -> re.Pattern[str]:
    ...
    - **Whitespace after `<!--` is tolerated**, and anything following the slug is
      ignored — so re-wrapping or editing the explanatory parenthetical does not
      change whether the block is *there*. Matching the whole generated comment is
      exactly the mistake round 5 caught: changing `generated by` to
      `generated  by` left the sentinel and the generated rule body intact, yet
      the block read as gone and became revertable while still live.
    - **A longer slug must not match a shorter one** (`foo` must not find
      `foo-bar`'s block), hence the trailing "not a slug character" guard.
    - **The closing marker can never match**: it is `<!-- /neurobase:rule:…`, and
      `\\s*` does not span the `/`. A lone closing marker is therefore not
      liveness, which is what makes an opening sentinel the right token.
    """
    return re.compile(r"<!--\s*neurobase:rule:" + re.escape(slug) + r"(?![a-z0-9-])")
...
def candidate_is_live(text: str, doc: store.Document) -> bool:
    ...
    - **rule:** the slug's opening **sentinel** (`rule_sentinel`) is present, no
      matter what foreign bytes surround it and no matter how the explanatory
      parenthetical after it was re-wrapped or edited. An edit *outside* the
      block leaves it live; removing the sentinel makes it gone (the file may
      well remain, holding the user's own content). A lone *closing* marker is
      not liveness.
    """
    kind = str(doc.get("type") or "")
    slug = str(doc.get("name") or "")
    if kind == "skill":
        return True
    if kind == "rule":
        return rule_sentinel(slug).search(text) is not None
    return False
...
def _rule(root: Path, doc: store.Document, slug: str, draft: str) -> Artifact:
    ...
    block = f"{start}\n{draft.rstrip()}\n{end}"
    # Locate an existing block by the same SENTINEL the liveness check uses, not
    # by the full generated comment (review F1, round 5). Two reasons: the write
    # and the liveness verdict must never disagree about whether a block is
    # there; and a block whose explanatory parenthetical was hand-edited is then
    # *replaced* by the canonical one rather than duplicated beside it. We always
    # WRITE the full comment — tolerant detection, canonical output.
    matches = list(rule_sentinel(slug).finditer(before))
    start_count = len(matches)
    s = matches[0].start() if matches else -1
    s_end = matches[0].end() if matches else -1
    ...
    if start_count > 1 or end_count > 1 or (s >= 0) != (e >= 0) or (s >= 0 and e < s_end):
