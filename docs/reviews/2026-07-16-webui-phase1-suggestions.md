...
promote the surface to law (spec §14 + ADR-0021), record two live-smoke
...
- `docs/neurobase-spec-appendix.md` §14 + `docs/adr/0021-webui-surface.md` —
...
4. Spec §14 / ADR-0021 accuracy — does the written law match the shipped code
...
- `cli/__init__.py` lazily imports `webui.app.serve` in the `ui` command — the
  ...
  earlier security pass; ADR-0021 records it.
...
- **suggested direction:** reject non-loopback Host authorities (allowlist)
  before the origin comparison; extend §14/ADR-0021 so the allowlist is
...
- **resolution:** resolved — `is_loopback_host` allowlist
  ...
  §14 + ADR-0021 extended; tests cover the exact rebinding shape
