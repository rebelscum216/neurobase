...
**`redact_command(text, extra_patterns)`** — a second public entry point, for a value the caller *knows* is a shell command: spec §4's tool-activity digest, where the scribe captured Claude's `input.command` verbatim. It runs `redact`, then redacts **every** secret-named assignment in the string case-insensitively, with no keyword or position required — so `api_token=… ./run.sh` and `pytest --api-key=…` both go. Knowing the channel is precisely what licenses that aggression: a command is not prose and not code, so we can be strict there without taxing every prompt and assistant message. `adapters/claude/scribe.py` uses it for the `### Commands run` bullets and plain `redact` for everything else.
