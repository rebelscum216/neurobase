...
- `curate(root: Path, project: str, brain: Brain, *, dry_run: bool = False, resynth: bool = False, tombstone_grace_days: int = DEFAULT_TOMBSTONE_GRACE_DAYS, plan_payload_max_bytes: int = DEFAULT_PLAN_PAYLOAD_MAX_BYTES, distill: str = DEFAULT_DISTILL, distill_chunk_chars: int = DEFAULT_DISTILL_CHUNK_CHARS, redact_patterns: tuple[str, ...] = ()) -> dict[str, Any]` — runs one full pass of the spec §2 sequence and returns the summary dict (the function re-exported as `curate` from `neurobase.curator`, imported by the CLI as `run_curate`). Control flow:
  ...
  16. **Fold record** (ADR-0022 B2). Whenever at least one batch committed — the same gate as steps 13/14, so it rides the after-commit *error* path too — `_log_pass` is given a `fold` object: `{v, consumed[], edges{}, superseded[], tombstoned[]}`, built from the accumulators in 9–12. It is deliberately **not** a summary key: the CLI prints the returned summary as JSON and `fold.consumed` carries raw `session_id`s, so the fold rides the on-disk journal record *only* (loopback-only sensitivity). It is absent when nothing committed (first-batch abort, noop) and on every dry-run path.
