...
- `_SHELL_ENV_SECRET` → case-insensitive `\b(export|declare|typeset|local|setenv|env)((?:[ \t]+-\S+)*[ \t]+)(<SECRET_NAME>)[ \t]*=[ \t]*\S+` → an assignment in an explicit **shell assignment context**: `export API_TOKEN=…`, `env api_key=… pytest`, `declare -x my_secret=…`. Keeps the keyword and the name. The line-anchored rule alone never matched these, which meant `export API_TOKEN=<secret>` — the most common shape a secret takes in a shell command — went unredacted throughout v0.1; it became *reachable* when the Claude scribe started capturing a command digest (ADR-0013). This rule can safely stay **case-insensitive** because the *keyword* is what marks it as a variable assignment: shell names are case-sensitive but nothing requires them to be uppercase, so `export api_token=…` leaks just as well.
- `_INLINE_ENV_SECRET` → `(?<![A-Za-z0-9_])(<SECRET_NAME>)[ \t]*=[ \t]*\S+` → a **bare** inline assignment with no keyword to disambiguate: `API_TOKEN=… cmd`, `make && API_TOKEN=…`. Here the *name's shape* is the only signal that this is a secret rather than a keyword argument, so this rule is deliberately **case-sensitive**: a case-insensitive version would swallow `items.sort(key=lambda x: x.id)` and `df.groupby(key=col, secret=False)`. Lowercase bare assignments are not lost — they are still covered when they open a line (`_ENV_SECRET`) or carry a keyword (`_SHELL_ENV_SECRET`). The three rules together are why casing is *not* the lever that separates secrets from code; **context** is.
