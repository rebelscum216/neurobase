...
**The accepted cost.** A credential-*named argument* to another command (`env PATH=/bin pytest api_key=example`, `awk -v api_key=x`) is redacted even though a perfect parser would leave it alone. It is fail-closed, the command's shape survives (`api_key=[REDACTED:env-secret]`), and across **2,699 real captured commands only ~1% contain a secret-named `name=` token at all** — so position tracking buys almost no fidelity in practice while costing correctness we could not deliver. `_SECRET_OPTION_NAMES` is an allow-list, never a `*key*` pattern: `--sort-key=name`, `--key=id`, `--password-policy=strict` select and configure, they do not authenticate.
