...
**`_scrub_shell(text)`** — the structural core, used by both the `_SHELL_SEGMENT` rule and `redact_command`. It walks the text's **shell tokens** (`_TOKEN` keeps quoted spans intact, so `a="b c"` is one word) and redacts a token only when it is an assignment *in assignment position*: in the command prefix (`api_token=… ./run.sh`), or anywhere in the word list of an assignment builtin (`env`/`export`/`declare`/…, which is what lets `env -u OLD api_token=… pytest` work — an option operand can't hide the assignments after it). Once a plain command word is seen, `assignment_zone` closes and the remaining words are that command's arguments. Secret-*named* options (`--api-key=…`) are redacted in any position. `_redact_assignment` keeps the name verbatim, quoting and all, and drops the whole value token.
