...
- **Placement rule — `curated/` is authoritative (spec §1).** Either or both of `curated/<slug>.md` and `.tombstones/<slug>.md` may exist; when both do, the slug is **active** and the tombstone is residue. `list_curated` reads only `curated/`, and the recommender's evidence resolver checks `curated/` before `.tombstones/`, so current readers satisfy this for free — but any *new* reader of `.tombstones/` MUST exclude slugs present in `curated/`. Nothing on a write path reclaims residue; only `prune_tombstones` does, once past the grace period. Note `prune_tombstones` has a known TOCTOU against a concurrent soft-delete (known-gaps **G4**), so the store's no-loss property holds for a single mutating process, not for concurrent ones.
