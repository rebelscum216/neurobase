...
**Idempotence is a contract, not a nicety.** `redact` runs more than once over the same text — the scribe scrubs each captured value, then the whole assembled document as defense in depth. `[REDACTED:env-secret]` contains no word-break character, so a value scanner that doesn't recognize the marker reads `[REDACTED:env-secret]export …` as a single bare token and replaces it, **silently eating the following word**. So a marker is matched as a value — but only when it is the *complete* value: a marker *prefix* (`api_token=[REDACTED:env-secret]SECRET`) must still redact the suffix, or a marker pasted into captured text becomes an escape hatch for the secret beside it. Configured `extra_patterns` obey the same rule: existing markers are opaque, nonempty matches run to a fixed point, and zero-width matches are ignored because they contain no captured text. Both marker/value bugs were real, and the first was caught by the idempotence check in `scripts/audit_command_redaction.py` — the one real property that script can test without an oracle.
...
**`redact_command(text, extra_patterns)`** — the public entry point for a value the caller *knows* is a shell command: spec §4's tool-activity digest, where the scribe captured Claude's `input.command` verbatim. It runs `redact`, then `_scrub_shell`. `adapters/claude/scribe.py` uses it for the `### Commands run` bullets and plain `redact` for everything else.
